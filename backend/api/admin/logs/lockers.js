...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { page = '1', limit = '20', lockerId, userId, action, method, startDate, endDate, search } = req.query;
    ...
    try {
      ...
      console.error('[admin/logs/lockers GET]', err);
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      const result = await prisma.lockerLog.deleteMany({ where });
      return res.status(200).json({ success: true, count: result.count });
    } catch (err) {
      console.error('[admin/logs/lockers DELETE]', err);
      return res.status(500).json({ error: 'Internal server error' });
    }
  }
